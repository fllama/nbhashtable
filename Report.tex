...
\begin{abstract}
The abstract goes here. \hl{Should we have a real abstract? This was in the template, but I'm not sure.}
\end{abstract}
...
\IEEEPARstart{S}{oon} we will have to replace this first section with a bold, grand introduction of this wonderful paper. \hl{Insert a real introduction here.}
...
Our implementation of the overall data structure has many similarities to the pseudocode provided in the paper, but has several changes to keep it sequential.

The structure is encapsulated in a C++ class called NBHashTable. \hl{Maybe a whole section about choosing C++11 and why? It's in our title, after all.} The internal storage structure of the hashtable is an integer array ‘buckets’ that represents the hashtable bucket locations. The size of the array is determined by an integer argument passed into the NBHashTable constructor method. This value is assigned to an internal size variable ‘kSize’ and is used throughout the hashtable class, including for the instantiation of the buckets array.\hl{Talk about static-ness of kSize and future possibility to allow growing hashtables}

%\subsection{C++11 thread libraries}

Initially the bucket array locations are initialized to the value of the preprocessor macro EMPTY\_FLAG defined in the NBHashTable header file. In our implementation it is set to -1 to represent an empty bucket. \hl{Why is it set to -1? What does that mean about insertions?}

The bounds values associated with each bucket location are represented as an integer array initialized to be the same size as the buckets array. Each location in the array is initially initialized to 0 to represent that there have been no hash collisions, and we do not need to keep jumping to find our value.

Values added or removed to the hash table are expected to be an integer greater than or equal to 0. Before each addition or removal of an integer value, the value is passed into a hash function to make an associated index that is within the range of this buckets and bounds array. This hash function is simply (value \% kSize) where kSize is the hashtable size.

The methods used to interact with the buckets are similar to those described in the pseudocode, with one method to obtain values from the bucket, and one method to test whether a given collision exists. Both take a ‘startIndex’ argument representing the hashed version of an integer value to be added or removed, and a ‘probeJumps’ integer representing the length of the probe sequence needed to find the value. A pointer to a bounds location is returned when getting a bucket value so that the value can be changed without having to access the the bucket array twice.

When inserting or removing a value from the hashtable, the first index location in the buckets array that is checked is the hashed version of that value. When inserting, if there is already a value in that position in the array, other locations in the area are checked to be empty so it can be inserted there instead. The locations checked in this probing sequence are determined by the quadratic formula '(startIndex + (probeJumps * (probeJumps + 1)) /2'. This probing is done in a for-loop with a counter called ‘probeJumps’, which is iterated until it is greater than the kSize constant or an empty location is found at an index defined by the quadratic equation.\hl{As stated in other section, these paragraphs are too similar to the fist part, we're gonna have to do some adjusting on what we're talking about where now that the paper is more put-together}

When removing or locating an item, the same probing sequence is done, except the for-loop stops when the counter is greater than the bounds value at the original hash value location (or when the value its searching for is found first).
...
To keep the algorithm parallel and look-free, the pseudocode provided uses lock-free atomic compare-and-swap functions when access and modifying bucket and bounds values. The paper also specifies packing a state, a version count, and a bucket value into a single word value for each bucket for atomic access and modification. These modifications make the structure lock-free and parallel, so our implementation does not use them in order to keep the structure sequential.\hl{This paragraph should be put into first section, and instead we should talk just about the sequential-ness of our implementation here.}

\subsection{Sequential-ness}

\hl{Some rearranging and editing goes here.}

Instead, we use a single mutex that is locked for every public method call and unlocked at the end of every public method. This guarantees no two threads will be interacting with the hash table at the same time. If one thread is interacting with the hash table and another thread tries to simultaneously, it will have to wait until the first thread is finished and unlocks the mutex. This guarantees sequential performance.
